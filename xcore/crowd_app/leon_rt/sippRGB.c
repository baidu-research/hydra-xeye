...
extern XeyeBoardInfo_t xeye_board_info;
ALIGNED(64) uint32_t DDR_AREA histLuma[256];
//The full-frame buffers
UInt8      DDR_DATA SippRGB_dmaOut0_buffer   [SIPP_ISP_PIPE_BUFFER_WIDTH* SIPP_ISP_PIPE_BUFFER_HEIGHT* 3  ]ALIGNED(8);
UInt8       DDR_DATA SippRGB_dmaOut1_buffer   [SIPP_ISP_PIPE_BUFFER_WIDTH* SIPP_ISP_PIPE_BUFFER_HEIGHT* SIPP_ISP_PIPE_BUFFER_INPUT_NUM_PLANES]ALIGNED(8);
...
{
   //sippWrFileU8((UInt8*)SippRGB_dmaOut0_buffer,SIPP_ISP_PIPE_BUFFER_WIDTH * SIPP_ISP_PIPE_BUFFER_HEIGHT * 3 * sizeof(UInt8),"output.raw");
   sippWrFileU8((UInt8*)SippRGB_dmaOut1_buffer,SIPP_ISP_PIPE_BUFFER_WIDTH * SIPP_ISP_PIPE_BUFFER_HEIGHT * SIPP_ISP_PIPE_BUFFER_INPUT_NUM_PLANES* sizeof(UInt8),"output1.raw");
   //Ensure parameter is used to avoid compiler warning
   (void)(pPl);
...
void buildSippRGB(SippRGB *pPl)
{
   ...
   // create filters
   pPl->dmaIn0          =  sippCreateFilter(pPl->pl, 0x00,       SIPP_ISP_PIPE_BUFFER_WIDTH, SIPP_ISP_PIPE_BUFFER_HEIGHT, N_PL(SIPP_ISP_PIPE_BUFFER_INPUT_NUM_PLANES), SZ(UInt16), SIPP_AUTO,          (FnSvuRun)SIPP_DMA_ID,        0);
   pPl->sigmaDnsMa2x5x0 =  sippCreateFilter(pPl->pl, 0x0,        SIPP_ISP_PIPE_BUFFER_WIDTH, SIPP_ISP_PIPE_BUFFER_HEIGHT, N_PL(SIPP_ISP_PIPE_BUFFER_INPUT_NUM_PLANES), SZ(UInt16), SIPP_AUTO,          (FnSvuRun)SIPP_SIGMA_ID,      0);
   pPl->lscMa2x5x0      =  sippCreateFilter(pPl->pl, 0x0,        SIPP_ISP_PIPE_BUFFER_WIDTH, SIPP_ISP_PIPE_BUFFER_HEIGHT, N_PL(SIPP_ISP_PIPE_BUFFER_INPUT_NUM_PLANES), SZ(UInt16), SIPP_AUTO,          (FnSvuRun)SIPP_LSC_ID,        0);
   pPl->rawMa2x5x0      =  sippCreateFilter(pPl->pl, 0x0,        SIPP_ISP_PIPE_BUFFER_WIDTH, SIPP_ISP_PIPE_BUFFER_HEIGHT, N_PL(SIPP_ISP_PIPE_BUFFER_INPUT_NUM_PLANES), SZ(UInt8), SIPP_AUTO,          (FnSvuRun)SIPP_RAW_ID,        0);
   pPl->debayerMa2x5x0  =  sippCreateFilter(pPl->pl, 0x0,        SIPP_ISP_PIPE_BUFFER_WIDTH, SIPP_ISP_PIPE_BUFFER_HEIGHT, N_PL(3),  SZ(UInt8), SIPP_AUTO,          (FnSvuRun)SIPP_DBYR_ID,       0);
   sippFilterAddOBuf(pPl->debayerMa2x5x0, SIPP_ISP_PIPE_BUFFER_INPUT_NUM_PLANES, sizeof(UInt8));
   pPl->dmaOut0         =  sippCreateFilter(pPl->pl, 0x00,       SIPP_ISP_PIPE_BUFFER_WIDTH, SIPP_ISP_PIPE_BUFFER_HEIGHT, N_PL(3),  SZ(UInt8), SIPP_AUTO,          (FnSvuRun)SIPP_DMA_ID,        0);
   pPl->dmaOut1         =  sippCreateFilter(pPl->pl, 0x00,       SIPP_ISP_PIPE_BUFFER_WIDTH, SIPP_ISP_PIPE_BUFFER_HEIGHT, N_PL(SIPP_ISP_PIPE_BUFFER_INPUT_NUM_PLANES), SZ(UInt8), SIPP_AUTO,          (FnSvuRun)SIPP_DMA_ID,        0);
...
void configSippRGB(SippRGB *pPl)
{
   ...
   if ((xeye_board_info.sensor_type == Sensor_AR0330)||(xeye_board_info.sensor_type == Sensor_AR0144))
  ...
  // fields as defined in the filter property window
   ...
   sigmaDnsMa2x5x0Cfg->blcGR=64;
   sigmaDnsMa2x5x0Cfg->blcR=64;
   sigmaDnsMa2x5x0Cfg->blcB=64;
   sigmaDnsMa2x5x0Cfg->blcGB=64;
   ...
   rawMa2x5x0Cfg->cfg=      RAW_CFG(BAYER,BAYER_PATTERN_locate,GRGB_IMBAL_EN,0,AE_PATCH_STATS_EN,AE_RGB_HIST_STATS_EN,(ISPC_BAYER_BITS - 1),ISPC_GRGB_IMBAL_THRESHOLD,0,AF_PATCH_STATS_EN,BAYER_2x2_MODE,AE_Y_HIST_STATS_EN,HOT_COLD_PIX_SUPPRESS_EN);
    if (xeye_board_info.sensor_type == Sensor_AR0330) {
        rawMa2x5x0Cfg->gainSat[0]=((ISPC_RAW_CLAMP_0<<16) | ISPC_RAW_GAIN_GR_AR0330 );
        rawMa2x5x0Cfg->gainSat[1]= ((ISPC_RAW_CLAMP_1<<16) | ISPC_RAW_GAIN_R_AR0330 );
        rawMa2x5x0Cfg->gainSat[2]= ((ISPC_RAW_CLAMP_2<<16) | ISPC_RAW_GAIN_B_AR0330 );
        rawMa2x5x0Cfg->gainSat[3]= ((ISPC_RAW_CLAMP_3<<16) | ISPC_RAW_GAIN_GB_AR0330 );
    } else if(xeye_board_info.sensor_type == Sensor_AR0144)
    {
        rawMa2x5x0Cfg->gainSat[0]=((ISPC_RAW_CLAMP_0_AR0144<<16)  | ISPC_RAW_GAIN_GR_AR0144 );
        rawMa2x5x0Cfg->gainSat[1]= ((ISPC_RAW_CLAMP_1_AR0144<<16) | ISPC_RAW_GAIN_R_AR0144 );
        rawMa2x5x0Cfg->gainSat[2]= ((ISPC_RAW_CLAMP_2_AR0144<<16) | ISPC_RAW_GAIN_B_AR0144 );
        rawMa2x5x0Cfg->gainSat[3]= ((ISPC_RAW_CLAMP_3_AR0144<<16) | ISPC_RAW_GAIN_GB_AR0144 );
    }
    else
    {
   ...
   debayerMa2x5x0Cfg->cfg=  DEBAYER_CFG(BAYER_PATTERN_locate,0,0,10,8,P_RGB, 3,0);
