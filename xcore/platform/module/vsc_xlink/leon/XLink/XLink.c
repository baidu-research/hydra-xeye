...
int handleIncomingEvent(xLinkEvent_t* event){
 ...
 int dispatcherEventReceive(xLinkEvent_t* event){
    ...
    if (USBLinkIsReconnect()) {
        USBLinkResetReconnectFlag();
        return 0;
    }
            ...
            prevEvent.xLinkFD == event->xLinkFD)
    {
        ...
        // TODO(hyx): It doesn't matter when duplicate id is detected, so just let program continue
        // ASSERT_X_LINK(0);
        // TODO(hyx): We can not ignore USB_PING_REQ event, it's a real event from remote
        if (event->header.type != USB_PING_REQ) {
            return 0;
        }
    }
    prevEvent = *event;
...
XLinkError_t XLinkInitialize(XLinkGlobalHandler_t* handler)
{
    ...
    printf("UsbLinkPlatformInit finish \n");
    ...
    printf("dispatcherInitialize finish \n");
#ifndef __PC__
    int index = getNextAvailableLinkIndex();
    if (index == -1)
        return X_LINK_COMMUNICATION_NOT_OPEN;
